# 🕌 ISLAMIC TECH — نسخة Cloudflare (Pages + Functions + D1 + R2)

نسخة كاملة الأركان محل نسخة Google Apps Script القديمة، فيها:
...
## ⚠️ اقرأ ده الأول — حدود حقيقية مش نواقص في الكود

1. **مقدرش أعمل حسابات نيابةً عنك.** إنشاء حساب Cloudflare، Google Cloud،
   Fawry، أو أي بوابة دفع لازم يتعمل بإيميلك/رقمك/بيانات نشاطك التجاري
   الحقيقية. الخطوات تحت هتوصلك لحسابك في دقايق.
2. **"يتحمل مليار طالب" مش رقم تقني ثابت.** المعمارية هنا (Cloudflare edge +
   D1 + R2) بتتوسّع فعليًا وأد ما تحتاج، لكن في حدود حسابك (تكلفة، حدود
   الخطة). لو المنصة كبرت جدًا هتحتاج تراقب استهلاك D1/R2 وتترقّى للخطط
   المدفوعة المناسبة.
3. **مفيش تقنية ويب تمنع تصوير الشاشة 100%.** أي حد يقدر يصور شاشته بموبايل
   تاني مهما عملنا حماية. اللي اتعمل هنا هو **ردع حقيقي**:
...
**رابط لوحة الأدمن بقى معقّد ومخبّي عمدًا (مش /admin.html):**
```
...
## ⚠️ ليه فيه فولدرات جوّه functions/ ومش ممكن أشيلها
...
لو فرّطنا الملفات كلها جنب بعض من غير فولدرات، كل الروابط هتتغيّر
(هتبقى كلها `/index`, `/index`, `/index`...) والموقع هيقف تمامًا —
ده على الأرجح سبب إن النسخة اللي رفعتها قبل كده على Cloudflare "ما
اشتغلتش ومقرتش المسارات": لو الفولدرات اتفرطت أو اتحطت غلط وقت الرفع.

**الحل الحقيقي لمشكلة "تعب إنشاء الفولدرات يدوي على GitHub":**
مش محتاج تعمل ولا فولدر واحد بإيدك خالص:
